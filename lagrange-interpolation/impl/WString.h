...
namespace Utility {
	...
	public:
		...
		StringErrorCode ErrorCode() const
		{
			return num;
		}
	...
	public:
		...
		{
			//return !wcscmp(m_ptr, str); //!memcmp(m_ptr, str, m_length << 1); 크기를 모르므로 막 검사하면안된다.
			// wcscmp는 날코딩 되어있다.
	...
	private:
		...
		size_t FindLastHelper(const wchar_t *str, size_t ends, size_t len) const
		{
			...
			size_t len2 = len  * sizeof(wchar_t);

			// 루틴: fast-wcsrstr

			// ULL은 느리다(왠지 모르겠다)
			uint32_t wide_checker = ((uint32_t)*str << 16) | ((uint32_t)*str);

			// wide_checker가 루프에선 중복되므로
			auto wcsrnchr = [&] (const wchar_t *ptr, size_t len) -> const wchar_t *
			{
				const wchar_t *lptr = ptr + len - 1;
				const wchar_t *aptr = (wchar_t*)((uint32_t)lptr & -4L);
				
				if (aptr < ptr) aptr = ptr;
				
				for ( ; lptr >= aptr; --lptr )
					if (*lptr == *str) return  lptr;
				
				for (lptr = aptr - 2; lptr >= ptr; lptr -= 2)
				{
					uint32_t nptr = *(uint32_t *)lptr ^ wide_checker;

					if ( HawzHelper(nptr) )
					{
						if (lptr[1] == *str) return lptr + 1;
						return lptr;
					}
				}

				for (lptr += 3; lptr >= aptr; --lptr)
					if (*lptr == *str) return  lptr;

				return NULL;
			};

			while (ptr = wcsrnchr(m_ptr, searchLen))
			{
				searchLen = ptr - m_ptr;
				if (!memcmp(ptr, str, len2)) // wmemcmp는 피킹해보면 알겠지만 날코딩되어있다. ㄷㄷ;;
					return searchLen;
			}
	...
	public:
		...
		size_t FindLast(const wchar_t ch, size_t ends = 0) const
		{
			...
			const wchar_t *ptr = wcsrnchr(m_ptr, ch, m_length - ends);
...
#define _CONTAINS_HELPER_SKIP(t,ch) {\
	...
	private:
		...
		// strt2는 첫 번재 요소의 처리가 마지막으로 
		// 끝나면 루프문이 종료되어야되므로 ptr과 대조시켜진 것임
		// 이것은 str-1을 가리켜야 한다.
		SplitsArray SplitReverseHelper(const wchar_t *strt2, const wchar_t *last, size_t max)
		{
			wchar_t      **position = new wchar_t*[m_length];
			size_t        *poslen = new size_t[m_length];
			size_t         count = 0;
			size_t         internalc = 0;
			const wchar_t *s1, *s2;
			const wchar_t *ptr = m_last;
			const wchar_t *ptrt2 = m_ptr - 1;
			const wchar_t *tlast = m_last;

			// 그나저나 이 함수를 사용하는 사람이 몇이나될까? -> wcsrnstrn를 써도될듯
			for( ; ptr != ptrt2 && max; ptr-- )
			{
				s1 = ptr;
				s2 = last;
				internalc++;
				while (s1 != ptrt2 && s2 != strt2 && !(*s1 - *s2))
					s1--, s2--;
				if (s2 == strt2)
				{
					// 바로 위에서 inernalc++를 해주었으니 -1를 더 해주어야함
					// (_internalc - 1) -> -_internalc + 1
					position[count] = (wchar_t *)tlast - internalc + 2;
					poslen[count] = internalc - 1;
					count++;
					internalc = 0;
					ptr = s1 + 1;
					tlast = s1;
			...
			if (max_remain)
			{
				// internalc의 값이 맨 마지막 루프에서 통과되었으므로
				n[count] = new WString((const wchar_t *)(tlast - internalc + 1), internalc);
			}
	...
	public:
		...
		SplitsArray SplitReverse(const wchar_t *str, size_t max = SIZE_MAX)
		{
			return SplitReverseHelper(str - 1, str + wcslen(str) - 1, max);
		}
		
		SplitsArray SplitReverse(const WString& refer, size_t max = SIZE_MAX)
		{
			return SplitReverseHelper(refer.m_ptr - 1, refer.m_last, max);
		}
	...
	public:
		...
		WString PadLeft(size_t len, wchar_t pad = L' ')
		{
			if (len > m_length)
			{
				...
				const wchar_t *p = m_ptr;
				size_t i = 0;

				for ( ; i < len - m_length; i++)
					ret[i] = pad;
				for ( ; i < len; i++, p++)
					ret[i] = *p;
...
#define _ZWIF(n) if(HawzHelper(trim[n])) {trim += n; break;}
	...
	_X_BRU( _K(3) _K(4) )
		...
		wchar_t *wcsrnchr(wchar_t *ptr, wchar_t ch, size_t len) const
		{
	   ...
	   _X_BRU ( ((checker_type)ch << 32) | )
		...
		wchar_t *wcsrnstrn(wchar_t *ptr, const wchar_t *dest, size_t ptrlen, size_t destlen)
		{
			wchar_t *tptr;
			size_t len2 = destlen  * sizeof(wchar_t);
			uint32_t wide_checker = ((uint32_t)*dest << 16) | ((uint32_t)*dest);

			auto wcsrnchr = [&] (wchar_t *ptr, size_t len) -> wchar_t *
			{
				wchar_t *lptr = ptr + len - 1;
				wchar_t *aptr = (wchar_t*)((uint32_t)lptr & -4L);
				
				if (aptr < ptr) aptr = ptr;
				
				for ( ; lptr >= aptr; --lptr )
					if (*lptr == *dest) return  lptr;
			...
			while (tptr = wcsrnchr(m_ptr, ptrlen))
			{
				ptrlen = tptr - m_ptr;
		...
		void wcsnset(wchar_t *ptr, wchar_t ch, size_t len)
		{
